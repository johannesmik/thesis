...
if __name__ == '__main__':
    ...
    # Cluster Image
    centers, labels, inertias, ratios = cluster(color_image, depth_image, ir_image,
                                                lambdas=[.5, .5/3, .5, 0], threshold=0.8, k_min=1, k_max=20, all_clusters=False)

    plot_clusters(centers, labels)

    #plot_inertias_ratio(inertias, ratios, minmax=(1,20))
    ...
    # Extract
    angles_per_cluster, intensities_per_angle = extract_angles(ir_image, centers, labels, bins=20, binrange=(0, 60), normal_image=normals)

    plot_angle_intensities(angles_per_cluster, np.array(intensities_per_angle) / 255., centers)

    # Fit
    popts = fit_clusters_to_phong(angles_per_cluster, np.array(intensities_per_angle) / 255.)

    print 'popts',  popts

    plt.figure()
    plt.title('Plots ...')
    codebook = get_codebook(centers)
    # Plot the found functions
    for i, popt in enumerate(popts):
        angles_cont = np.linspace(0, 90, 120)
        plt.plot(angles_cont, phong2(angles_cont, *popt), color=cluster_cmap(codebook[i,0]))
